...
class Conv3d(nn.Module):
    def __init__(self):
        ...
        self.pool_time = nn.AvgPool3d(kernel_size=(20, 1, 1), stride=(10, 1, 1))

        self.conv_time2 = nn.Conv3d(64, 128, (6, 1, 1))
        self.conv_spat2 = nn.Conv3d(128, 256, (1, 2, 2))
        self.pool_time2 = nn.AvgPool3d(kernel_size=(20, 1, 1), stride=(10, 1, 1))
...
class FBCSP2(nn.Module):
  def __init__(self):
    super(FBCSP2, self).__init__()
    self.time_normalization = time_normalization
    self.conv_time = nn.Conv2d(1, 32, (36, 1))
    self.dropout1 = nn.Dropout2d(p=0.1)
    self.conv_spat = nn.Conv2d(32, 32, (1, 64), bias=False)
    self.dropout2 = nn.Dropout2d(p=0.2)
    self.batchnorm2 = nn.BatchNorm2d(32)
    self.pool = nn.AvgPool2d(kernel_size=(36, 1), stride=(12, 1))
  ...
  def forward(self, x):
    x = self.time_normalization(x, time_dim=2)
    h = F.tanh(self.dropout1(self.conv_time(x)))
    h = F.tanh(self.dropout2(self.conv_spat(h)))
    h = self.batchnorm2(h)
    h = self.pool(h)
    h = self.dropout3(h)
    h = F.tanh(self.conv_class(h))
    h = self.score_ave(h)
    # print(h.size())
    # return h
    return h.squeeze()
...
def cv_train(model_class, criterion_class, optimizer_class, X, y,
    ...
    for train_idx, val_idx in kf.split(X=X, y=y):
        ...
        print('train_batch:{}  val_batch:{}'.format(len(train_y),len(val_y)))
...
class Trainer(object):
  ...
  def model_saver(self):
    torch.save(model.state_dict(), 'weight_linear.pth')

def main(cv_splits, batch_size, epochs, model_class, crop, lr, subject_id=[1,2,3],
    ...
    X, y = make_class(subject_id=subject_id, crop=crop, problem=taskname)
    if conv1d:
        pass
    else:
        if map2d:
    ...
    np.save('all_sub_accuracies/task({})model({})_acc'.format(taskname, name)+'.npy', acc)
...
if __name__ == '__main__':
    ...
    conv3d = True
    conv1d = False
    model_name = "ConvLSTM_allsub" #Conv3d_convLSTM, FBCSP, Conv
